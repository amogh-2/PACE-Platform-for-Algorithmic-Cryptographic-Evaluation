document.addEventListener("DOMContentLoaded", () => {
  console.log("DOM fully loaded and parsed - AES-256-GCM");
  ...
  if (runButton) {
      runButton.addEventListener("click", async () => {
          console.log("Run Benchmark clicked - AES-256-GCM");
          ...
          try {
              ...
              const response = await fetch("/run_benchmark", {
                  ...
                  body: JSON.stringify({ fileSize: selectedSize, algorithm: "AES-256-GCM" }),
              });
  ...
  async function loadExistingResults() {
      try {
          const selectedSize = fileSize.value;
          const response = await fetch(`/get_benchmark_results?algorithm=AES-256-GCM&fileSize=${selectedSize}`);
